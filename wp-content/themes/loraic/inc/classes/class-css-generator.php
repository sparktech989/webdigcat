...
class Loraic_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = loraic()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'loraic_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'loraic_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->loraic_generate_file_options();
		} );
	}

	function loraic_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->loraic_generate_file_options();
			$this->loraic_generate_file();
		}
	}

    function loraic_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->loraic_options_output() )
        ) );
    }

	function loraic_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function loraic_options_output() {
		$theme_colors                    = loraic_configs('theme_colors');
        $links                           = loraic_configs('link');
        $gradients                       = loraic_configs('gradient');
		ob_start();

		printf('$loraic_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$loraic_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function loraic_enqueue() {
        $css = $this->loraic_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function loraic_inline_css() {
        ob_start();
