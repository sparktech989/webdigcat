...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'loraic');
...
$merlin_setup = get_option( 'merlin_' . loraic()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'loraic');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'loraic');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'loraic');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'loraic' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'loraic' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'loraic' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Loraic_Admin_Plugins;
