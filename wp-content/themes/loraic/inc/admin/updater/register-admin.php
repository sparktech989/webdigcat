<?php
/**
 * The Loraic_Register initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly

class Loraic_Register {
	...
	public function __construct( $config = array(), $strings = array() ) {
		$config = wp_parse_args( $config, array(
			...
			'theme_slug'     => loraic()->get_slug(),
			'theme_name'     => loraic()->get_name(),
			'version'        => '',
	...
	function updater() {
		...
		if ( !class_exists( 'Loraic_Updater' ) ) {
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'loraic' ) ), loraic()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
	}
	
	function admin_notice() {
		echo '<div class="notice"><p>'.esc_html__( 'Purchase code is invalid. Need a license for activation', 'loraic' ).'</p></div>';
	}
	...
	function messages($merlin = false){
		...
		if ( ! $purchase_code ){
			...
			<div class="pxl-dsb-box-head-inner">
				<h6><?php echo esc_html__( 'Register License', 'loraic' ) ?></h6>
			</div>
			<?php 
			$this->form();
			?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'loraic' ); ?></a>
			</div>
			<?php 
		}else{  
	...
	function check_license($merlin) {
		...
		if ( false === $license_data->success ) {
			switch ( $license_data->error ) {
				...
				$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'loraic' ), $this->theme_name );
				break;
				case 'license_exists':
			...
			<div class="pxl-dsb-confirmation fail">
				<h6><?php echo esc_html__( 'Active false', 'loraic' ) ?></h6>
				<p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'loraic' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'loraic' ) ?></a></p>
			</div>
			<?php $this->form(); ?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'loraic' ); ?></a>
			...
			if ( 'valid' === $license_data->license ) {
				...
				<div class="pxl-dsb-box-head"> 
					<div class="pxl-dsb-confirmation success">
						<h6><?php echo esc_html__( 'Thanks for the verification!', 'loraic' ) ?></h6>
						<p><?php echo esc_html__( 'You can now enjoy and build great websites. Looking for help? Visit', 'loraic' ) ?> <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'loraic' ) ?></a>.</p>
					</div> 
					...
					<div class="pxl-dsb-deactive">
						<form method="POST" action="<?php echo admin_url( 'admin.php?page=pxlart' )?>">
							...
							<button class="btn button" type="submit"><?php esc_html_e( 'Remove Purchase Code', 'loraic' ) ?></button>
						</form>
					</div> 
				</div> 
				<?php 
				...
				$message = esc_html__( 'Response return null.', 'loraic' ); ?>
				<div class="pxl-dsb-confirmation fail">
					<h6><?php echo esc_html__( 'Active false', 'loraic' ) ?></h6>
					<p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'loraic' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'loraic' ) ?></a></p>
				</div>
	...
	function form() {
		...
		<form action="options.php" method="post" class="pxl-dsb-register-form">
			...
			<input id="<?php echo esc_attr($this->theme_slug)?>_purchase_code" name="<?php echo esc_attr($this->theme_slug)?>_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'loraic' ); ?>">
			<input type="submit" class="res-purchase-code" value="<?php esc_attr_e( 'Register your purchase code', 'loraic' ) ?>">
		</form>
		<?php
	}
