<?php if($settings['search_type'] == 'popup') : ?>
	...
	<?php add_action( 'pxl_anchor_target', 'loraic_hook_anchor_search'); ?>
<?php endif; ?>
...
<?php if($settings['search_type'] == 'form') : ?>
	<form role="search" method="get" class="pxl-widget-searchform" action="<?php echo esc_url(home_url( '/' )); ?>">
		<div class="searchform-wrap">
			<input type="text" placeholder="<?php if(!empty($settings['email_placefolder'])) { echo esc_attr($settings['email_placefolder']); } else { esc_attr_e('Search...', 'loraic'); } ?>" name="s" class="search-field" />
