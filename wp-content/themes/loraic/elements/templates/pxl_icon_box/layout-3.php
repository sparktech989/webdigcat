<div class="pxl-icon-box pxl-icon-box3 <?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms">
    <div class="pxl-item--inner">
        <?php if (!empty($settings['icon_bg_image']['id']) ) : ?>
            <?php $img_icon  = pxl_get_image_by_size( array(
                'attach_id'  => $settings['icon_bg_image']['id'],
                'thumb_size' => 'full',
            ) );
            $thumbnail_icon    = $img_icon['thumbnail'];
            ?>
            <div class="box-hover" style="background-image: url('<?php echo esc_attr($img_icon['url']) ?>');"></div>
        <?php endif; ?>
