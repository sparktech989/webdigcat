...
$post_type = $widget->get_setting('post_type', 'carrer');
$layout = $widget->get_setting('layout_'.$post_type, 'carrer-1');


$show_excerpt = $widget->get_setting('show_excerpt');
$button_apply = $widget->get_setting('button_apply');
...
<div id="<?php echo esc_attr($html_id) ?>" class="pxl-grid pxl-carrer-grid pxl-carrer-grid-layout1 pxl-carrer-style1" data-layout="masonry" data-start-page="<?php echo esc_attr($paged); ?>" data-max-pages="<?php echo esc_attr($max); ?>" data-total="<?php echo esc_attr($total); ?>" data-perpage="<?php echo esc_attr($limit); ?>" data-next-link="<?php echo esc_attr($next_link); ?>">
    ...
    <div class="<?php echo esc_attr($grid_class); ?>" data-gutter="15">
        ...
        <?php $load_more['tax'] = $tax; loraic_get_post_grid($posts, $load_more); ?>
    </div>
    <?php   
    ...
    if ($pagination_type == 'pagination') { ?>
        <div class="pxl-grid-pagination" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>" data-query="<?php echo esc_attr(json_encode($args)); ?>">
            <?php loraic()->page->get_pagination($query, true); ?>
        </div>
    <?php } ?>
    <?php if (!empty($next_link) && $pagination_type == 'loadmore') { ?>
        <div class="pxl-load-more pxl-load-more-style1" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>">
            <span class="btn">
                <?php echo esc_html__('Load More', 'loraic') ?>
